  ...
  return (
    <div className="flex items-center">
      {sources?.map((source, index) => (
        <div
          key={source?.chain?.id}
          className={cn("rounded-full transition-transform hover:scale-110", index > 0 && "-ml-2")}
...
const ViewHistory = ({
      ...
      return (
        <>
          {displayedHistory?.map((pastIntent) => (
            <Card
              key={pastIntent.id}
              className="!mx-4 !p-6 hover:shadow-md transition-shadow duration-200 border-border/100 gap-3">
              ...
              <div
                ...
                <div
                  className="flex items-center justify-between gap-x-3 flex-1 w-full sm:min-w-fit">
                ...
                <div
                  className="flex items-center justify-between sm:justify-end  gap-x-2 w-full">
  ...
  if (!viewAsModal) {
    return (
      <div
        className="flex flex-col gap-y-3 max-h-96 no-scrollbar overflow-y-auto w-full max-w-md">
  ...
  return (
    <Dialog>
      ...
      <DialogContent className="max-w-2xl flex flex-col">
        <DialogHeader>
          <DialogTitle className="text-2xl font-semibold !p-4">
            Transaction History
          </DialogTitle>
        </DialogHeader>
        <div
          className="flex flex-col gap-y-3 max-h-96 no-scrollbar overflow-y-auto w-full">
          {renderHistoryContent()}
        </div>
      </DialogContent>
    </Dialog>
