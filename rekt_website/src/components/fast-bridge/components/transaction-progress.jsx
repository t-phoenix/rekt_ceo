...
export const getOperationText = (type) => {
  switch (type) {
    case "bridge":
      return "Transaction";
    case "transfer":
...
const StepList = memo(({ steps, currentIndex }) => {
  return (
...
const TransactionProgress = ({
  ...
  const { effectiveSteps, currentIndex } = useMemo(() => {
    ...
    const thresholds = milestones.map((_, idx) => (idx + 1) / milestones.length);
    const displaySteps = milestones.map((label, idx) => ({
      id: `M${idx}`,
      label,
    ...
    <div className="w-full flex flex-col items-center">
      <div className="flex flex-col items-center gap-y-3">
        ...
        <div className="flex items-center justify-center w-full">
          ...
          <span
            className="text-base font-semibold font-nexus-primary text-nexus-muted-secondary">
